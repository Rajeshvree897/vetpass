...
export default (app: Router) => {
  app.get("/stores", authRedirect(true), (req: Request, res: Response) => {
  ...
  app.get('/store', authRedirect(true), async (req: Request, res: Response) => {
    const commonService = new CommonService();
   
    if(req.query["country"]) {
      return res.json(await commonService.getStates(req.query["country"]));
    }
    return res.render('stores/vet-practice.ejs', {
      title: constants.PAGE_TITLE.STORE_ADD,
      userData: req.session && req.session.user ? req.session.user : null,
      ...
      classificationData: await commonService.getStoreClassifications(),
      petService: await commonService.getPetServices()
    });
  });

  app.get("/favoriteStores", authRedirect(true), (req: Request, res: Response) => {
    res.render("stores/favorite-vet-practices.ejs", {
      title: "Favorite Store list",
      userData: req.session && req.session.user ? req.session.user : null
    });
  });

  app.get(
    "/favoriteStores/downloadReport",
    authRedirect(true),
  ...
  app.get("/favoriteStore/data", authRedirect(true), (req: Request, res: Response) => {
  ...
  app.post("/checkStoreEmail", authRedirect(true), async (req: Request, res: Response) => {
  ...
  app.get("/store/data", authRedirect(true), (req: Request, res: Response) => {
    let whereObj = {};
    if(req.session && req.session.user && req.session.user.roleName && req.session.user.roleName === config.common.role.vet_practice_admin_role_name){
      whereObj = { created_by: req.session.user.id, is_deleted: null, option: "store" };
    }else{
      whereObj = { is_deleted: null, option: "store" }
    }
  ...
  app.get("/stores/:id/delete", authRedirect(true), (req: Request, res: Response, next: NextFunction) => {
    const vetPracticesService = new VetPracticesService();
    vetPracticesService
  ...
  app.get("/stores/:id/unClaim", authRedirect(true), (req: Request, res: Response, next: NextFunction) => {
    const vetPracticesService = new VetPracticesService();
    vetPracticesService
      ...
      .then(() => {
        req.flash(
          "success",
          "Vet Store unClaimed successfully.",
          req,
          res
        );
        res.redirect("/admin/stores");
  ...
  app.get(
    ...
    (req: Request, res: Response, next: NextFunction) => {
      ...
      .then(async (responseData: any) => {
          res.render("stores/vet-practice.ejs", {
            moment,
            title: constants.PAGE_TITLE.STORE_EDIT,
            userData: req.session && req.session.user ? req.session.user : null,
            responseData,
            ...
            classificationData: await commonService.getStoreClassifications(responseData.VetPracticesStoreClassification, true),
            petService: await commonService.getPetServices(responseData.petService, true)
          })
        })
        .catch((error: Error) => {
          return next(error)
  ...
  app.get(
    ...
    (req: Request, res: Response, next: NextFunction) => {
      ...
      vetPracticesService
        ...
        .then(async (responseData: any) => {
          res.render("stores/vet-practice-profile.ejs", {
            moment,
            title: constants.PAGE_TITLE.STORE_EDIT,
            userData: req.session && req.session.user ? req.session.user : null,
            responseData,
    ...
    (req: Request, res: Response, next: NextFunction) => {
      ...
      vetPracticesService
        ...
        .then(() => {
          ...
          return res.redirect("/admin/vetPracticeProfile");
        })
        .catch(async (error: Error) => {
          if(error.message === 'Email already exist') {
            const commonService = new CommonService();
            req.flash('error', error.message, req, res);
            return res.render("stores/vet-practice-profile.ejs", {
              moment,
              title: constants.PAGE_TITLE.STORE_EDIT,
              userData: req.session && req.session.user ? req.session.user : null,
  ...
  app.post(
    ...
    async (req: any, res: Response, next: NextFunction) => {
      const vetPracticesService = new VetPracticesService();
      vetPracticesService
        .importVetStore(req.file, req.session?.user.id, req.session?.user.timeZone)
        .then(() => {
          req.flash(
            "success",
  ...
  app.get("/reportedStores", authRedirect(true), (req: Request, res: Response) => {
    res.render("stores/reported-practices.ejs", {
      title: "Reported Store list",
      userData: req.session && req.session.user ? req.session.user : null
    });
  });

  app.get(
    "/reported-stores/data",
    authRedirect(true),
